...
- [ ] 1.1 Add Figma's twelve `space/*` values to `MonetaSpacing` under Figma's
...
- [ ] 2.1 Implement `MonetaButton` from `13:2` — 5 styles × 3 sizes × 3 states,
...
- [ ] 2.2 Implement `MonetaPaginationDots` from `70:223`. Verify:
...
- [ ] 4.1 Download the six decorative illustration SVGs and declare them. Verify:
  a test loads each from the bundle, as the icon set already does.
- [ ] 4.2 Implement the controller: read the flag, record completion on finish or
  ...
  and a failing store.
- [ ] 4.3 Implement the three slides and the splash, transcribing copy, spacing
