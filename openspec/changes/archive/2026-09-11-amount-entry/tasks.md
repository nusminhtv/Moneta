...
- [x] 1.1 `Currency.groupSeparator`, `decimalSeparator` and `symbolLeads`,
...
- [x] 1.2 `Money.parse` takes the locale's separators, and grouping must be
...
- [x] 2.1 A grouping `TextInputFormatter` in
...
- [x] 2.2 The sheet's amount field uses it and shows the currency on the side
...
- [x] 3.1 Record in `docs/design-system/figma-map.md` that the add sheet is
...
- [x] 3.2 Full `tool/verify.sh --change amount-entry` green with its evidence
